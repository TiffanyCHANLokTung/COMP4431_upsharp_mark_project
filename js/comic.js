(function(imageproc) {
    ...
    imageproc.comicColor = function(inputData, outputData, saturation) {
        ...
        for (var i = 0; i < inputData.data.length; i += 4) {
            ...
            var {h,s,v} = imageproc.fromRGBToHSV(r,g,b);
            s = Math.min(Math.max(s * saturation, 0), 1);
            var newRGB = imageproc.fromHSVToRGB(h,s,v);
            r = newRGB["r"];
            g = newRGB["g"];
            b = newRGB["b"];
            ...
            var minDistance = Math.hypot(256, 256, 256);
            var Distance;
            for (var j = 0; j < palette.length; j++)
            {
                Distance = Math.hypot(palette[j][0] - r, palette[j][1] - g, palette[j][2] - b);
