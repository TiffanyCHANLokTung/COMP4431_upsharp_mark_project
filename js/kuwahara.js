(function(imageproc) {
    ...
    imageproc.kuwahara = function(inputData, outputData, size) {
        ...
        var subRegionEdgeToCenter = (subRegionSize - 1) / 2;
        var subRegionDivisor = subRegionSize * subRegionSize;

        function regionStat(x, y) {
            // Find the mean colour and brightness
            var meanR = 0, meanG = 0, meanB = 0;
            var meanValue = 0;
            for (var j = -1 * subRegionEdgeToCenter; j <= subRegionEdgeToCenter; j++) {
                for (var i = -1 * subRegionEdgeToCenter; i <= subRegionEdgeToCenter; i++) {
            ...
            meanR /= subRegionDivisor;
            meanG /= subRegionDivisor;
            meanB /= subRegionDivisor;
            meanValue /= subRegionDivisor;
        ...
        for (var y = 0; y < inputData.height; y++) {
            for (var x = 0; x < inputData.width; x++) {
                ...
                var regionA = regionStat(x - subRegionEdgeToCenter, y - subRegionEdgeToCenter, inputData);
                var regionB = regionStat(x + subRegionEdgeToCenter, y - subRegionEdgeToCenter, inputData);
                var regionC = regionStat(x - subRegionEdgeToCenter, y + subRegionEdgeToCenter, inputData);
                var regionD = regionStat(x + subRegionEdgeToCenter, y + subRegionEdgeToCenter, inputData);
